...
- 静态图片壁纸（`Image`，aspect-crop 填充）与视频动态壁纸（`MediaPlayer` + `VideoOutput`，循环、静音）；视频在 N 卡机器上强制走 **GStreamer 后端**以启用 NVDEC 硬解——Qt 6.8 默认的 FFmpeg 后端按线程数分配解码 surface，4K 片源会超过驱动的 32 个上限，`cuvidCreateDecoder` 失败后静默回退软解（0.60 core vs 硬解的 0.27），详见 `src/main.cpp`；显式设置 `QT_MEDIA_BACKEND` 可覆盖
- **随机动态壁纸**：控制中心里可以添加多个动态视频，壁纸类型选「动态视频（随机）」后**每次上锁**从列表里随机抽一个来播，且不会连续两次抽到同一个。列表为空、或列表里的文件都不在，回退内置壁纸；多屏共享同一次抽签结果
- **时间与日期位置可调**：控制中心里用横向/纵向两个百分比（0–100，50 = 居中 = 旧版位置）调整时间日期这一组的位置；位置调过之后，出现密码框时时间日期会在当前位置渐隐、并在屏幕正中渐显
